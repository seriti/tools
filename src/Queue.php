...
class Queue extends Model 
{
        ...
        if($queue === 0) {
            ...
            foreach($queue as $id => $item) {
                ...
                if(is_array($update)) {
                    ...
                    if(isset($update[$this->queue_cols['queue_id']])) unset($update[$this->queue_cols['queue_id']]);
    ...
    public function processQueueSql($process_id,$param) {
        ...
        $sql='SELECT T.'.$this->queue_cols['queue_id'].' AS id , T.* '.
        ...
        $sql .= 'ORDER BY '.$this->queue_cols['queue_id'].' LIMIT '. $param['max_items'];
        return $sql;
    }
